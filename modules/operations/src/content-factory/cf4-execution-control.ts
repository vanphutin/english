...
/**
 * CF4-specific durable execution controls that were intentionally absent from
 * the first CF4 coordinator. Attempts are real DB attempts (1..3), and run
 * budget reservations are atomic so concurrent workers cannot overspend the
 * same run limits.
 */
export class Cf4ExecutionControl {
  ...
  /**
   * Creates one immutable attempt. A retry must use attempt 2 or 3 rather than
   * mutating/re-queuing attempt 1, preserving all prior evidence.
   */
